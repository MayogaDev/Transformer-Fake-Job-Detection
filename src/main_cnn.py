...
def main():
    """
    Main function to load and preprocess data, create Dataset and DataLoader objects, initialize CNN model, loss function, and optimizer,
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_val_df)):
        ...
        model = CNNClassifier(vocab_size, embed_dim, n_filters, filter_sizes, output_dim, dropout).to(device)
    ...
    results_df.to_csv('cnn_training_results.csv', index=False)
    print("Results saved to 'cnn_training_results.csv'")
